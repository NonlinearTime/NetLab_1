...
void Receicer::getIpAddress(QHostAddress addr,quint16 port)
{
    this->addr = addr;
    this->port = port;
}
...
void Receicer::setFileName(QString filename)
{
    if(file.isOpen())
        return;
    ...
    if(!file.open(QIODevice::WriteOnly|QIODevice::Append|QIODevice::Unbuffered))
        return;
    //ReceivedFile::GetInstance()->fileMaxSize = file.size();
}

void Receicer::initSocket(quint16 listenport)
{
