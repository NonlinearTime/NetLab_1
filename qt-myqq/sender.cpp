...
void Sender::initSocket(quint16 listenport)
{
    qDebug() << listenport;
    udpSocket = new QUdpSocket(this);
    //udpSocket->setReadBufferSize(1024 * 1024);
    udpSocket->bind(listenport,QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);
...
void Sender::getFileName(const QString &fileName)
{
    ...
    SendedFile::GetInstance()->fileMaxSize = file.size();
}
...
void Sender::getIpAddress(QHostAddress ipAddr, quint16 port)
{
    ...
    tail = head + windowSize -1;
    ackedRec.clear();
    //for (int i = 0 ; i != 100000 ; i++) ackedRec[i] = NULL;
    //qDebug() << "15";
    sendFile();
}
...
void Sender::timeoutResend()
{
    ...
    if (udpSocket->bytesAvailable()!=0) {
       qDebug() << "socket failes, restart!";
        udpSocket->close();
        initSocket(PublicSocket::GetInstance()->getClientPort());
    }
    qDebug() << "------------resending------------";
